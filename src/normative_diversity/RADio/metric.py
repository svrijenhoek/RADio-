...
class DiversityMetric:
    ...
    def __init__(self, **kwargs):
        ...
        self.recommendation_builder = DistributionBuilder(
            self.feature_type, self.rank_aware_recommendation
        )
        self.context_builder = DistributionBuilder(
            self.feature_type, self.rank_aware_context
        )
        ...
        if self.context_type == "dynamic":
            ...
            if not self.Q_static:
                if self.feature_type == "cont":
                    self.context_builder.bins_discretizer.fit(
                        np.array(context).reshape(-1, 1)
                    )
                    self.recommendation_builder.bins_discretizer.fit(
                        np.array(context).reshape(-1, 1)
                    )
