...
def distplot_all(df, algorithms, metric):
    ...
    fig = ff.create_distplot(data, algorithms, bin_size=0.01, show_hist=False)
    fig.show()
...
def lineplot(df, metric):
    ...
    df_melted = hourly_avg.melt(
        id_vars="per_hour", var_name="algorithm", value_name="average_value"
    )
    fig = px.line(
        df_melted,
        x="per_hour",
        y="average_value",
        color="algorithm",
        title="Average Values for " + metric + " Over Time",
    )
    fig.show()
...
def seaborn_per_hour(df, metric):
    ...
    sns.lineplot(x="per_hour", y=metric, hue="algorithm", data=df)


def visualize_metric(df, metric, algorithms):
